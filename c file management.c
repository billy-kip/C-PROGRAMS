...
int writeParagraph() {
    FILE *fptr = fopen("C:\\Users\\Admin\\Desktop\\My c programs\\output.txt", "a");
    if (fptr == NULL) {
    ...
    printf("Paragraph written to %s successfully\n","C:\\Users\\Admin\\Desktop\\My c programs\\output.txt");
    return 0;
}

// Function to read contents from the file
int readFile() {
    FILE *fptr = fopen("C:\\Users\\Admin\\Desktop\\My c programs\\output.txt", "r");
    if (fptr == NULL) {
